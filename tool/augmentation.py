...
def collater(data):
    ...
    imgs = torch.from_numpy(np.stack(imgs, axis=0)).type(torch.float32)

    annot_padded = torch.cat(annots, 0).type(torch.float32)
...
class Resizer(object):
    ...
    def __call__(self, sample, common_size=608):
...
class Augmenter(object):
    ...
    def __call__(self, sample, flip_x=0.5):
        if np.random.rand() < flip_x:
            ...
            cen_x, cen_y, w, h = annots[:, 1], annots[:, 2], annots[:, 3], annots[:, 4]
            ...
            new_x1, new_x2 = (cols - x2), (cols - x_tmp)
            new_cenx = (new_x1 + new_x2) / 2
